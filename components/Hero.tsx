...
// Unique and attractive text for all dynamic elements
const herox = [
    { 
        id: 1, 
...
export default function Hero() {
    ...
    // Auto-rotate slides and text every 5 seconds
    useEffect(() => {
    ...
    // CSS class to apply the dynamic animation to all content
    ...
    return (
        // Hero takes 60% of the viewport height
        <section className="relative h-[60vh] md:h-[60%] w-full overflow-hidden">
            
            {/* Dynamic Background Images container */}
            ...
            {/* Content Container - Items remain centered but spacing is tighter */}
            <div className="relative z-20 flex flex-col items-center justify-center h-full text-center px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto py-8">
                ...
                {/* Animated Headline - Slightly reduced max size for better fit */}
                <div className="overflow-hidden mb-4 py-1">
                    <h1 
                        className={`font-extrabold text-white text-5xl sm:text-6xl md:text-7xl lg:text-7xl leading-tight drop-shadow-lg ${contentAnimationClass}`}
                    >
                        {currentHero.label}
                    </h1>
                </div>

                {/* Animated Sub-text - Reduced margin below */}
                <p className={`text-sm sm:text-xl text-sky-200 mb-6 max-w-2xl font-light drop-shadow-md ${contentAnimationClass}`}>
                    {currentHero.p_text}
                </p>

                {/* Animated Call to Action Button - Consistent padding */}
                <a
                    href="#book-now"
                ...
                {/* Indicator Dots - Moved closer to the bottom edge */}
