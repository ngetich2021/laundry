...
export default function Book({ isOpen, onClose }: BookProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      setSubmitError(emailError.text || 'Failed to send. Try again.');
    } finally {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="relative w-full max-w-lg bg-white rounded-lg shadow-xl p-6 animate-in fade-in zoom-in duration-200">
          {/* Close Button */}
          <button
            onClick={onClose}
          ...
          {submitSuccess && (
            <div className="mb-4 p-3 bg-green-100 text-green-700 rounded">
              Success! Your booking was sent. We’ll call you soon.
            </div>
          )}
          {submitError && (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            {/* Phone */}
            <div>
              <label className="block text-sm font-medium text-gray-700">Phone (Kenyan)</label>
              <input
                {...register('phone')}
                type="tel"
            ...
            {/* Message */}
            <div>
              <label className="block text-sm font-medium text-gray-700">Pickup Details / Message</label>
              <textarea
                {...register('message')}
                rows={4}
